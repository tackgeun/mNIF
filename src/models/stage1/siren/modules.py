...
def get_model(model_type, sparse_type, num_hidden_layers, hidden_features, image_resolution, weight_gp=0.0, out_features=3, in_features=2):
    # define the model.
    if model_type == 'sine' \
            or model_type == 'relu' \
            or model_type == 'tanh' \
            or model_type == 'selu' \
            or model_type == 'elu' \
            or model_type == 'softplus'\
            or model_type == 'gaussian':
        model = SingleBVPNet(type=model_type, mode='mlp', sidelength=image_resolution,
                             sparse_matrix=sparse_type,
                             in_features=in_features,
                             out_features=out_features,
                             hidden_features=hidden_features,
                             num_hidden_layers=num_hidden_layers,
                             weight_gp=weight_gp)
    elif model_type == 'rbf' or model_type == 'nerf' or model_type == 'hashgrid' or model_type == 'ffnet':
        model = SingleBVPNet(type='relu', mode=model_type, sidelength=image_resolution,
                             in_features=in_features,
                             out_features=out_features,
                             sparse_matrix=sparse_type,
                             hidden_features=hidden_features,
...
class GaussianAct(nn.Module):
    ...
    def forward(self, input):
        return torch.exp(-(input ** 2)/(2*self.a_square))
...
class TriangleWave(nn.Module):
    ...
    def forward(self, x):
        return (4/self.p) * (x - (self.p/2) * torch.floor( (2*x)/self.p + 0.5 ) ) * (-1) ** torch.floor(2*x/self.p + 0.5)
...
class FCBlock(MetaModule):
    ...
    def __init__(self, in_features, out_features, num_hidden_layers, hidden_features,
        ...
        w0 = 30
        if 'k-mixtures' in nonlinearity:
            k_mixtures = int(nonlinearity.split('k-mixtures')[-1])
            nonlinearity = nonlinearity.split(str(k_mixtures))[0]
        elif 'sine-w0=' in nonlinearity:
            w0 = float(nonlinearity.split('sine-w0=')[-1])
            nonlinearity = nonlinearity.split(str(w0))[0]
        elif 'sine-ffnet' in nonlinearity:
            w0 = float(nonlinearity.split('sine-ffnet')[-1])
            nonlinearity = nonlinearity.split(str(w0))[0]
        elif 'trianglewave' in nonlinearity:
            w0 = float(nonlinearity.split('trianglewave')[-1])
            nonlinearity = nonlinearity.split(str(w0))[0]
        elif 'scalerelu' in nonlinearity:
            w0 = float(nonlinearity.split('scalerelu')[-1])
            nonlinearity = nonlinearity.split(str(w0))[0]

        nls_and_inits = {'sine':(Sine(), sine_init, first_layer_sine_init),
                         'sine-w0=':(Sine(w0), lambda m: sine_init(m, w0), first_layer_sine_init),
                         'sine-ffnet':(Sine(w0), lambda m: sine_init(m, w0), lambda m: sine_init(m, w0)),
                         'trianglewave': (TriangleWave(w0), lambda m: sine_init(m, w0), lambda m: sine_init(m, w0)),
                         'relu':(nn.ReLU(inplace=True), init_weights_normal, None),
                         'scalerelu':(ScaleReLU(w0), init_weights_normal, init_weights_normal),
                         'relu_k-mixtures':(nn.ReLU(inplace=True), lambda m: init_weights_normal_kmixture(k_mixtures, m), lambda m: init_weights_normal_kmixture(k_mixtures, m)),
                         'relu_uniform':(nn.ReLU(inplace=True), None, None),
                         'sigmoid':(nn.Sigmoid(), init_weights_xavier, None),
                         'tanh':(nn.Tanh(), init_weights_xavier, None),
                         'selu':(nn.SELU(inplace=True), init_weights_selu, None),
                         'softplus':(nn.Softplus(), init_weights_normal, None),
                         'elu':(nn.ELU(inplace=True), init_weights_elu, None),
                         'gaussian':(GaussianAct(), None, None)}
...
class SingleBVPNet(MetaModule):
    ...
    def __init__(self,
        ...
        elif self.mode == 'ffnet':
            self.positional_encoding = RandomFourierFeature(in_features=in_features,
                                                            num_frequencies=num_frequencies_ffnet,
                                                            sigma=sigma_ffnet)
    ...
    def forward_coordinate(self, model_input):
        ...
        elif self.mode in ['nerf', 'ffnet']:
            coords = self.positional_encoding(coords)
        elif self.mode == 'hashgrid':
            IB = coords.size(0)
            IC = coords.size(1)
    ...
    def forward(self, model_input, params=None):
        coords, coords_org = self.forward_coordinate(model_input)
    ...
    def gradient_penalty(self, inputs, mse_loss):
        batch_size = inputs.size(0)
        #outputs = self.net(inputs, self.net.get_subdict(params, 'net'))

        # Calculate gradients of probabilities with respect to examples
        gradients = torch_grad(outputs=mse_loss, inputs=inputs,
                               grad_outputs=torch.ones(mse_loss.size()).cuda(),
        ...
        return self.weight_gp * ((gradients_norm - 1) ** 2).mean()

    def forward_with_image_mse(self, model_input, gt, params=None):
        coords, coords_org = self.forward_coordinate(model_input)
        if params is None:
            params = OrderedDict(self.named_parameters())

        outputs = self.net(coords, self.net.get_subdict(params, 'net'))

        losses = {'img_loss': ((outputs - gt['img']) ** 2).mean()}
        model_outputs = {'model_in': coords_org, 'model_out': outputs}

        if self.weight_gp > 0.0:
            losses['gp_loss'] = self.gradient_penalty(coords,losses['img_loss'])
...
class RandomFourierFeature(nn.Module):
    '''Module to add random Fourier feature in FFNet [Tancik et al. 2020].'''
    def __init__(self, in_features, num_frequencies=256, sigma=1.0, use_gamma=True):
        super().__init__()
        ...
        if use_gamma:
            self.gamma = torch.nn.Parameter(torch.randn((1, 1, in_features, num_frequencies))*sigma)
        
        self.out_dim = num_frequencies * 2

    def forward(self, coords, gamma=None):
        if self.use_gamma:
            gamma = self.gamma

        coords = coords.view(coords.shape[0], -1, 1, self.in_features)
        x_proj = torch.matmul((2.*math.pi*coords), gamma).squeeze(2)
        sin = torch.sin(x_proj)
        cos = torch.cos(x_proj)

        embed = torch.cat([sin, cos], dim=2)
        return embed.detach()
...
def init_weights_normal_kmixture(k, m):
    if type(m) == BatchLinear or type(m) == nn.Linear:
        if hasattr(m, 'weight'):
            nn.init.kaiming_normal_(m.weight, a=0.0, nonlinearity='relu', mode='fan_in')
            #m.weight.data = m.weight * math.sqrt(k)
            #num_input = m.weight.size(-1)
            #nn.init.normal_(m.weight, std=( math.sqrt(k) / math.sqrt(num_input)))
            #nn.init.normal_(m.weight, std=( 1 / math.sqrt(num_input)))
            #nn.init.normal_(m.weight, std=( 1 / math.sqrt(num_input) / math.sqrt(k)))
